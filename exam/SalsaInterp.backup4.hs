...
command (Move shapeIds pos) = 
    ...
    ((), M.mapWithKey (\key a -> 
            ...
            state)
command (At com i) = undefined --Do local state of view(s) i (can be group)
command (Par c1 c2) = --execute c1 and c2 concurrently. May not manipulate same shape.
    SalsaCommand $ \state -> ((),
                              ...
                              in if M.difference diff1 diff2 == M.empty then
    ...
    where c1' = command c1
          c2' = command c2
          getState1 s = let (_, state1) = runSC c1' s in state1
          getState2 s = let (_, state2) = runSC c2' s in state2
