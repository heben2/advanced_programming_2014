...
liftC sc = 
    ...
    where 
        ...
        calcFrames s1 s2 env = 
            --for each list of frames in the list, combine first elems(gpxInstr) to one list of gpxInstr, that is, a frame.
            map concat $
            L.transpose $
            M.elems $ --[[frame]], list of list of frame pr shape
            M.mapWithKey  
                (\sName vps1 -> case M.lookup sName s2 of
                        ...
                        Nothing -> error "Something went wrong. Cannot find shape in both key frames"
                ) s1
        ...
        getShapeFrames :: Shape -> [(ViewName, Position)] -> [(ViewName, Position)] -> Integer -> [Frame]
        ...
        getShapeGpxInstrs :: Shape -> ViewName -> Position -> Position -> Integer -> [GpxInstr]
        getShapeGpxInstrs s vn pos1 pos2 fr = 
            case s of
                NewRectangle _ _ w h c -> 
                    map (\(x', y') -> DrawRect x' y' w h vn c) posList
                NewCircle _ _ r c -> 
                    map (\(x', y') -> DrawCirc x' y' r vn c) posList
...
definition (Rectangle i e1 e2 e3 e4 c) = Salsa $ 
    \con@(Context env state) (f:fs) -> 
        let e1'' = calcExpr e1 con --Hlint is silly...
            ...
            f' = f ++ map (\vn -> DrawRect e1'' e2' e3' e4' vn c') (aViews env)
        in
        ((),
...
definition (Circle i e1 e2 e3 c) = Salsa $ 
    \con@(Context env state) (f:fs) -> 
        let e1' = calcExpr e1 con
            ...
            v = map (\ident -> (ident,(e1', e2'))) $ aViews env --Insert on active views
            state' = M.insert i v state
            f' = f ++ map (\vn -> DrawCirc e1' e2' e3' vn c') (aViews env)
        in
        ((),
         Context (Environment (views env) (groups env) 
                  shapes' (aViews env) (framerate env)) state',
...
runProg f prog(p:ps) = (viewList, L.reverse frames)
    where (_, Context env state, frames) = runS (defCom p) (emptyContext f) emptyFrames
