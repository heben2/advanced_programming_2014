...
newtype Salsa a = Salsa {runS :: Context -> (a, Context, [Frame])}
    ...
    \con@(Context env state) -> 
        ((),
         let e1'' = calcExpr e1 con --Hlint is silly...
             ...
             shapes' = M.insert i (NewRectangle e1'' e2' e3' e4' $ getColor c) $ shapes env
             v = map (\ident -> (ident,(e1'', e2'))) $ aViews env --Insert on active views
             state' = M.insert i v state
         in Context (Environment (views env) (groups env) 
                  shapes' (aViews env) (framerate env)) 
            state'
        )
    ...
    \con@(Context env state) -> 
        ((),
         let e1' = calcExpr e1 con
             e2' = calcExpr e2 con
             e3' = checkDim $ calcExpr e3 con
             shapes' = M.insert i (NewCircle e1' e2' e3' $ getColor c) $ shapes env
             v = map (\ident -> (ident,(e1', e2'))) $ aViews env --Insert on active views
             state' = M.insert i v state
         in Context (Environment (views env) (groups env) 
                  shapes' (aViews env) (framerate env)) 
...
runProg i (p:ps) = undefined
--    where (_, con) = runS defCom p
